...
@Component({
  selector: 'app-mandate-on-call-rsv',
  templateUrl: './mandate-on-call-rsv.component.html',
  styleUrls: ['./mandate-on-call-rsv.component.css']
})
export class MandateOnCallRsvComponent implements OnInit {
  ...
  rsvfixing() {
    ...
    if ($('#sectionselector').val() == "USV") {
      ...
      this._mandateService.addleadhistory(rsvleadrsvfixparam).subscribe((success) => {
        this.status = success.status;
        if (this.status == "True") {
  ...
  rsvdonewithfixing() {
    ...
    this.autoremarks = " Scheduled the RSV again for " + this.selectedproperty_commaseperated + " On " + new Date($('#subrsvnextactiondate').val()).toDateString() + " " + $('#subrsvnextactiontime').val();
    var rsvleadrsvfixparam = {
      leadid: this.leadid,
